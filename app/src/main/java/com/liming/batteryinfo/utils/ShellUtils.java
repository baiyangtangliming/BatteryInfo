...
public final class ShellUtils {
    ...
    public static boolean haveRoot() {
        if (!mHaveRoot) {
            CommandResult  ret = execCmd("echo test",true); // 通过执行测试命令来检测
            if (ret.result != -1) {
                mHaveRoot = true;
            }
        }
    ...
    public static CommandResult execCmd(String[] commands, boolean isRoot, boolean isNeedResultMsg) {
        ...
        return new CommandResult(
                result,
                (successMsg == null || successMsg.length()< 1)? null : successMsg.deleteCharAt(successMsg.length()-1).toString(),
                (errorMsg == null || errorMsg.length()<1)? null : errorMsg.deleteCharAt(errorMsg.length()-1).toString()
        );
    }
