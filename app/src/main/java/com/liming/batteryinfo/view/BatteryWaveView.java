...
public class BatteryWaveView extends View {
    ...
    public void setmCurrentText(String mCurrentText) {
    ...
    public void setmPowerText(String mPowerText) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.BatteryWaveView);
        mBgColor = attributes.getColor(R.styleable.BatteryWaveView_bg_color, Color.parseColor("#FFF0F0F0"));
        mBorderColor = attributes.getColor(R.styleable.BatteryWaveView_border_color, Color.parseColor("#FFFFFFFF"));
        mFirstWaveColor = attributes.getColor(R.styleable.BatteryWaveView_first_wave_color, Color.parseColor("#FF4646F8"));
        mSecondWaveColor = attributes.getColor(R.styleable.BatteryWaveView_second_wave_color, Color.parseColor("#66266DFF"));
        mBubbleColor = attributes.getColor(R.styleable.BatteryWaveView_bubble_color, Color.parseColor("#33FFFFFF"));

        mCurrentText = attributes.getString(R.styleable.BatteryWaveView_currentText);
        mPowerText = attributes.getString(R.styleable.BatteryWaveView_powerText);

        mBorderWidth = attributes.getDimension(R.styleable.BatteryWaveView_boder_width, dip2px(6));
        mBubbleMaxRadius = attributes.getDimension(R.styleable.BatteryWaveView_bubble_max_radius, dip2px(10));
        mBubbleMinRadius = attributes.getDimension(R.styleable.BatteryWaveView_bubble_min_radius, dip2px(2));
        mBubbleMaxSize = attributes.getInteger(R.styleable.BatteryWaveView_bubble_max_size, 30);
        mBunbleMaxSpeedY = attributes.getInteger(R.styleable.BatteryWaveView_bubble_max_speed_y, 3);
        mProgress = attributes.getInteger(R.styleable.BatteryWaveView_progress, 60);

        float amplitudeRatioAttr = attributes.getFloat(R.styleable.BatteryWaveView_amplitude, 50f) / 1000;
    ...
    private void drawFont(Canvas canvas) {
        ...
        mTextPaint.getTextBounds(mCurrentText, 0, mCurrentText.length(), mBounds);
        int currentWidth = mBounds.width();
        int currentHeight = mBounds.height();

        canvas.drawText(mCurrentText, mWidth/2 - currentWidth/2, mWidth /2 -  currentHeight/2, mTextPaint);
        float power = mTextPaint.measureText(mPowerText);
        canvas.drawText(mPowerText, mWidth/2 - power / 2, mWidth /2 +  currentHeight*2, mTextPaint);
    }
    ...
    private boolean isOutOfRange(Bubble bubble) {
        ...
        float bw = getBetween(new Point((int) bubble.x, (int) bubble.y), new Point(mWidth / 2, mWidth / 2));
