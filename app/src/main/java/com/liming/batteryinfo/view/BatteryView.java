...
public class BatteryView extends View {
    ...
    private void drawVerticalBattery(Canvas canvas) {
        ...
        paint.setColor(mColor);
        paint.setStyle(Paint.Style.STROKE);
        float strokeWidth = height / 80.0f;
     ...
     * @param batterypower
     */
    private Integer getColor(int batterypower) {
        if (batterypower < 20) {
            return Color.parseColor("#EF5362");
        }else if (batterypower >= 20 && batterypower < 30) {
            return Color.parseColor("#FE6D4B");
        }
        else if (batterypower >= 30 && batterypower < 40) {
            return Color.parseColor("#FFCF47");
        }
        else if (batterypower >= 40 && batterypower < 50) {
            return Color.parseColor("#9FD661");
        }
        else {
            return Color.parseColor("#3FD0AD");
        }
    }
    /**
     * 设置电池电量
