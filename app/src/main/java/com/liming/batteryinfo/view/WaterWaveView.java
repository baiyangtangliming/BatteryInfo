...
public class WaterWaveView extends View {
	...
	private String flowNum = "+349mA";
	private String flowLeft = "电流";
	private String flowPower = "15.5W";
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(flowNum, mScreenWidth * 4 / 8 - num / 2, mScreenHeight /2 , flowPaint);
		float power = flowPaint.measureText(flowPower);
		canvas.drawText(flowPower, mScreenWidth * 4 / 8 - power / 2, mScreenHeight * 6 /8 , flowPaint);
		...
		// 绘制,即水面静止时的高度
        // 绘制,即水面静止时的高度
        RectF oval = new RectF((int)(mRingSTROKEWidth*1.5) , (int)(mRingSTROKEWidth*1.5) ,
                mScreenWidth - (int)(mRingSTROKEWidth*1.5) , mScreenHeight - (int)(mRingSTROKEWidth*1.5)) ;
        ...
        if (mWaterLevel > 0.50F) {
            startX = (int) (mRingSTROKEWidth + mCircleSTROKEWidth  + offsetWidth);
            endX = (int) (mScreenWidth - mRingSTROKEWidth*2 -  - offsetWidth);
        } else {
            startX = (int) (mRingSTROKEWidth*2-5  + offsetWidth - mAmplitude);
            endX = (int) (mScreenWidth - mRingSTROKEWidth*2 +5- offsetWidth + mAmplitude);
        }
        // 波浪效果
        while (startX < endX) {
            int startY = (int)
                    (f1 - mAmplitude * Math.sin(Math.PI * (2.0F * (startX + this.c * width * this.f)) / width));
            canvas.drawLine(startX, startY, startX, top+1, mWavePaint);
            startX++;
        }
        canvas.drawCircle(mScreenWidth / 2, mScreenHeight / 2, mScreenWidth / 2 - mRingSTROKEWidth, mRingPaint);

        canvas.drawCircle(mScreenWidth / 2, mScreenHeight / 2, mScreenWidth / 2 -mRingSTROKEWidth, mCirclePaint);
        canvas.restore();
	}
